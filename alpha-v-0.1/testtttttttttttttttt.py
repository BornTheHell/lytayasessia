...
class CaptchaApp:
    def __init__(self, root):
        ...
        self.max_attempts = 3  # Максимальное количество неверных попыток
    ...
    def check_answer(self):
        ...
        if user_answer == self.correct_answer:
            ...
            # Если ответ неверный, увеличиваем счетчик попыток
            self.incorrect_attempts += 1

            # Если достигнуто максимальное количество попыток, генерируем новую капчу
            if self.incorrect_attempts >= self.max_attempts:
                self.generate_captcha()
                self.incorrect_attempts = 0  # Сбрасываем счетчик
